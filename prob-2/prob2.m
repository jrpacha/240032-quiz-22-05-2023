...
fprintf('Quz 2022-23 Q2 P2\n')
fprintf('Temperature at internal boundary, tempIntBd = %.1f%cC\n',...
    tempIntBd,char(176))
fprintf('Temperature at external boundary, tempExtBd = %.1f%cC\n',...
    tempExtBd,char(176))
fprintf('Heat generation at R > %.0f, f = %.1f\n\n', rHeatGen, heatGen)
...
fprintf('Part (b)\n')
fprintf('Average of the Q values at the outer border (R = %.0f), <Q(outer)> = %.4e\n',...
    rMax, avQ)
fprintf('Hint-2. The number of elements in C2 is: %d\n\n',...
    numElem - numElemWithHeatGen)
fprintf('Part (c)\n')
fprintf('Averaged temperature in the annulus (%.0f < R < %.0f), <T(annulus)> = %.4e%cC\n',...
    R1, R2, avTempAnnulus,char(176))
